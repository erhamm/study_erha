...
interface ISchema {
  ...
  enumType?: { label: string; value: string }[]; // 枚举类型专用，枚举类型label和value的映射关系
}
...
const MyComponent: React.FC<Props> = ({ schema, dataSource }) => {
    ...
    schema.forEach((field) => {
      if (item[field.dataIndex] !== undefined) {
        if (field.type === Type.date) {
          ...
          const dateRange = (item[field.dataIndex] as number[])
            .map((date) =>
            ...
            .join(" - ");
          processedItem[field.dataIndex] = dateRange;
        } else {
  ...
  const columns = schema.map((field) => ({
    ...
    render: (text: string | number) => {
      if (field.type === Type.enumType) {
        const enumItem = field.enumType?.find(
